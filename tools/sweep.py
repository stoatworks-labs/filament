...
card and reports any that made no difference at all.
    ...
    python3 tools/sweep.py [--binary build/fitest] [--size WxH] [--jobs N]

Exit code 1 means something is dead.

------------------------------------------------------------------ the traps

**The card moves, and it flashes.** Wattage, Dimmer, Mains and Ambient Temp
are time controls: they change how fast and how far a filament follows its
drive, so on a still they would only show at the end of the warm-up. The
harness's card has a disc on an orbit, a drifting bar and a patch that
flashes on for 6 frames in 30, and the sweep renders 40 frames of it at a
synthetic 60 fps from a cold wall, so every one of them has an edge to act on.

**Gels needs colour.** An RGB-gelled wall drives each filament from one
channel; the card's colour patches are where that differs from luma.

**An option's range is its element count, an integer's is its real range.**
`fitest --list` prints both for exactly this reason, and the ends are what get
swept (Columns 4 and 160, Rows 2 and 90).
...
def main():
    ...
    ap.add_argument("--binary", default=str(ROOT / "build" / "fitest"))
    ...
    scratch = tempfile.mkdtemp(prefix="fisweep")
